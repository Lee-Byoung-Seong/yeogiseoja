...
import com.yeogiseoja.domain.notification.NotificationSubject;
...
public class OwnerServiceImpl implements OwnerService {
    ...
    @Override
    @Transactional
    public OwnerInfo.OwnerSummaryInfo confirmAuthOwner(OwnerCommand.ConfirmAuthRequest request) {
        var ownerEmail = request.getEmail();
        ownerValidator.validateConfirmAuthRequest(ownerEmail);
        var owner = ownerReader.getByEmail(ownerEmail);
        owner = owner.confirmAuthToken(request.getAuthToken());
